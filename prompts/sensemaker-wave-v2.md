...
本波结束时，必须对 `traits`、`motivation`、`capabilities`、`relationships`、`environment`、`narrative` 六个维度逐一评估。按共享基准判断每项变化，说明证据、情境与路线影响。没有新依据就不升级；依据被纠正或失效时，提出相应重算。一个来源可以关联多维，但每维需要独立说明，不能借一次回答制造虚假的覆盖。

即使某维度没有新证据、状态未变，也要为每个维度输出一条 `update_radar`（`from === to`），并简要说明当前证据状态；不能只更新看似“受影响”的维度而遗漏无变化的维度。

雷达表示理解所处的状态，不是人的成绩。不为了达到四维 `grounded` 而降低门槛。

### 4b. 同一次回答可以更新多个维度

本波结束时，在逐一完成六个维度评估的基础上，检查用户的回答是否改变了六维雷达中的多个维度。如果是，**为每个受影响的维度分别生成一条 `update_radar` operation / radar delta**，引用同一来源的不同面向，并独立说明证据在该维度上的含义。未受影响的维度仍要按 Step 4 输出 `from === to` 的 delta，不能省略。

- 不要合并成一条"综合雷达更新"。
- 例如用户说"我拒绝了升职，因为每周出差会挤掉陪家人的时间"：motivation、relationships、environment、narrative 都可能各有一条 delta。
- 每条 delta 的 `source_refs` 使用同一个 answer source，但 `reason` 要说明这条证据在该维度上具体意味着什么。
- 一个维度的证据越多越好：同一回答可以从多个角度为同一维度增加证据。不要因为"这个维度已经有 delta 了"就省略重复评估。证据的丰富度（具体场景数量、可观察行为次数、真实取舍次数）比维度覆盖的广度更重要。
...
9. 本波更新的 `radar_deltas` 是否分布在多个维度？是否把所有受影响维度都单独表达，而不是合并成一条模糊 delta？
10. 本波更新的 `radar_deltas` 是否覆盖全部六个维度？即使状态未变，每个维度是否都有对应的 `from === to` 的 delta？
