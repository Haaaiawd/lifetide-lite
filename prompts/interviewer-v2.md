...
### Step 5e: 基于完整历史记忆的波次演进

每个波次都应该基于全部历史记忆自然演进到新的调查方向，而不是重复已经充分探索过的方向。`selected_uncertainty` 是分析者给出的有益参考，但不是约束：如果它指向的方向与雷达缺口或历史中已经挖透的内容不一致，优先从历史中自然生长出新问题。

- 优先探索六维雷达中尚未触及或仅有线索的维度；
- 如果某个话题已经在前序波次得到具体场景和明确取舍，本波应避免再围绕它出大量新题；
- 波次推进是建议性的，不是硬性规则。若历史记忆显示某条线索仍有高价值、未澄清的张力，可以继续深挖，但要说明为什么这一题值得继续。
...
11. 本波问题是否基于完整历史记忆自然演进，没有重复已经充分回答或明确拒绝的方向？
