...
import type { RouteIntent, Prototype, ParallelLife, ParallelLivesPlan } from "@/lib/state/contracts";
...
export function buildPrototypesForPlan(
  ...
  return plan.lives.map((life) => buildPrototype(sessionId, provenanceIdToUse, life.trial_id, life.title));
}
...
function buildFallbackLifeFromRouteIntent(
  ...
  return {
    ...
    year_2: years.year_2,
    year_3: years.year_3,
    ordinary_day: `上午处理${intent.life_shape.responsibilities}，下午留出一小时用于${intent.title_hint}相关探索，晚上做简短记录。`,
    attractions: [
...
function buildFallbackParallelLivesPlan(
  sessionId: string,
  memory: WorkingMemory,
  provisional: boolean,
  provenanceId: string
...
export async function runSensemakerFinal(input: SensemakerFinalInput): Promise<SensemakerFinalOutput> {
  ...
  try {
    const raw = await generateStructured<ParallelLivesPlan>({
    ...
    plan = coerceEvidenceToActiveHeads(plan, input.memory);

    const validation = validateParallelLivesPlan(plan, input.memory);
    if (!validation.valid) {
      console.error("Final plan validation failed, using fallback:", validation.reason);
      return withPrototypes(buildFallbackParallelLivesPlan(sessionId, input.memory, input.provisional, provenanceId), sessionId, config);
    }

    return withPrototypes(plan, sessionId, config);
  } catch (err) {
